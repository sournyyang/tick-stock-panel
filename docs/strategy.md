...
**19 个内置策略**,每个策略一个独立 Python 文件,基于矩阵引擎向量化实现(`backend/app/strategy/builtin/`):

| 类型        | 代表策略                                                 |
| :---------- | :------------------------------------------------------- |
| 趋势 / 形态 | 趋势突破 · 均线多头 · MA 金叉 · MACD 金叉放量 · 布林突破 |
| 量价 / 涨停 | 量价齐升 · 试盘线战法 · 高换手强势 · 连板股 · 断板反包 · 涨停动量 · 接近涨停 |
...
每次成功完成策略回测后，系统会把完整结果和当时使用的参数保存到
`data/backtest_results/strategy/`。在回测工作台的「版本历史」中可以：

- 打开某次历史结果，继续查看收益、交易和每日记录；
- 恢复当时的日期、资金、成交、仓位、策略参数和环境过滤配置；
- 查看策略版本和代码指纹，判断历史记录是否仍由当前策略代码生成。

历史结果属于运行数据，由 Docker 数据卷持久化，不提交到 Git。如果页面提示记录来自旧策略代码，恢复参数后仍需切换到对应 Git 标签才能精确复现。项目发布版本使用 `vX.Y.Z` 标签，例如：

```bash
git fetch origin --tags
git switch --detach v0.1.89
docker compose up -d --build
```

需要回到最新开发版本时执行 `git switch main`，再重新构建即可。
